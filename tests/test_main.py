...
class TestMain:
    ...
    def test_main_basic_no_args(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
        """Test main() with no command line arguments — defaults to TUI."""
        with patch.object(sys, "argv", ["main.py"]):
            main.main()

        mock_load_dotenv.assert_called_once()
        mock_asyncio_run.assert_called_once()
        mock_run_tui.assert_called_once_with(skip_permissions=False, resume_id=None)

    @patch("main.load_dotenv")
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_main_with_skip_permissions_flag(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
        """Test main() with --dangerously-skip-permissions flag — defaults to TUI."""
        with patch.object(sys, "argv", ["main.py", "--dangerously-skip-permissions"]):
            main.main()

        mock_load_dotenv.assert_called_once()
        mock_run_tui.assert_called_once_with(skip_permissions=True, resume_id=None)
        mock_asyncio_run.assert_called_once()

    @patch("main.load_dotenv")
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_main_with_resume_and_id(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
        """Test main() with --resume and specific session ID — defaults to TUI."""
        with patch.object(sys, "argv", ["main.py", "--resume", "session123"]):
            main.main()

        mock_load_dotenv.assert_called_once()
        mock_run_tui.assert_called_once_with(skip_permissions=False, resume_id="session123")
        mock_asyncio_run.assert_called_once()

    @patch("main.load_dotenv")
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_main_with_resume_no_id(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
        """Test main() with --resume but no session ID (defaults to 'last')."""
        with patch.object(sys, "argv", ["main.py", "--resume"]):
            main.main()

        mock_load_dotenv.assert_called_once()
        mock_run_tui.assert_called_once_with(skip_permissions=False, resume_id="last")
        mock_asyncio_run.assert_called_once()

    @patch("main.load_dotenv")
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_main_with_resume_followed_by_flag(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
    ...
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_main_with_all_flags(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
        ...
        mock_load_dotenv.assert_called_once()
        mock_run_tui.assert_called_once_with(skip_permissions=True, resume_id="abc123")
        mock_asyncio_run.assert_called_once()
    ...
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_main_cancelled_error_handling(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
    ...
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_main_system_exit_handling(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
    ...
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_main_other_exception_propagates(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
...
class TestArgumentParsing:
    ...
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_resume_at_end_of_args(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
    ...
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_multiple_resume_flags(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
        """Test behavior with multiple --resume flags (uses first one)."""
        with patch.object(sys, "argv", ["main.py", "--resume", "first", "--resume", "second"]):
            main.main()

        mock_run_tui.assert_called_once_with(skip_permissions=False, resume_id="first")

    @patch("main.load_dotenv")
    @patch("aru.tui.run_tui")
    @patch("main.asyncio.run")
    def test_skip_permissions_position_variants(self, mock_asyncio_run, mock_run_tui, mock_load_dotenv):
        ...
        for args in test_cases:
            ...
            assert mock_run_tui.call_args[1]["skip_permissions"] is True