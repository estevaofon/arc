"""Multi-provider LLM abstraction for arc.
...
# ---------------------------------------------------------------------------
# Provider registry (built-ins + user overrides from arc.json)
# ---------------------------------------------------------------------------
...
def load_providers_from_config(config_data: dict[str, Any]):
    """Merge user-defined providers from arc.json into the registry.

    Expected format in arc.json:
    {
      "providers": {
        "ollama": {
          "base_url": "http://localhost:11434",
